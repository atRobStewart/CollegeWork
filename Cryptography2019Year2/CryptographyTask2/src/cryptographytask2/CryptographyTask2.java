...
public class CryptographyTask2{
        ...
        try{
            ...
            while (myFile.hasNextLine()){
                ...
                Matcher m = p.matcher(line); // finds illegal characters on the lines
                    ...
                    if (data.length != 3)continue; // skips lines that do not have two values entered
                    if (data[2].equalsIgnoreCase("0"))continue;
                    if (data.length == 3){
                        try{
//                            a = BigInteger.valueOf(lineCount);
                            BigInteger a = new BigInteger(data[0]); // changed int to long to deal with larger numbers
                            ...
                            System.out.println(getModPower(a, b, c));
    ...
    public static BigInteger power(BigInteger a, BigInteger b){
        if (b == BigInteger.ZERO){
            ...
            return a.multiply(power(a, b.subtract(BigInteger.ONE)));
        }
    }
    
    public static BigInteger getModPower(BigInteger a, BigInteger b, BigInteger c){
        BigInteger result; // will hold the value of a to the power of b
        a = a.abs();
        b = b.abs();
        result = power(a, b); // a to the ower of b is calculated
        result = result.abs();
        BigInteger remainder = result.subtract((result.divide(c)).multiply(c)); //finds the remainder of the result mod c 
        System.out.print("The " + a + "^" + b + " mod " + c + " = ");
